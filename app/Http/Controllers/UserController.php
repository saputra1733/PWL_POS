...
class UserController extends Controller
{
    ...
    //     coba akses model UserMode
    //     $user = UserModel::all();
    //     return view('user', ['data' => $user]);
    // }

    //Menampilkan halaman awaluser
    // public function index()
    // {
    //     $breadcrumb = (object) [
    //         'title' => 'Daftar User',
    ...
    //             $btn = '<a herf="'.url('/user/' .$user->user_id).'" class="btn btn-info btn-sm">Detail</a> ';
    //             $btn .= '<a herf="'.url('/user/' . $user->user_id . '/edit').'" class="btn btn-warning btn-sm">Edit<a> ';
    //             $btn .= '<form class="d-inline-block" method="POST" action="'.url('/user/'.$user->user_id).'">' 
    //                 . csrf_field() .method_field('DELETE') . 
    //                 '<button type="submit" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah Anda yakin menghapus data ini?\');">Hapus</button></form>';
    //             return $btn;
    //         })
    //     ->rawColumn(['aksi']) //memberitahu bahwa kolom aksi adalah html
    //     ->make(true);
    // }
    ...
    //         'username' => 'required|string|min:3|unique:m_user','username',
    //         'nama'     => 'required|string|max:100', // nama harus diisi, berupa string, dan maksimal 100 karakter
    //         'password' => 'required|min:5',          // password harus diisi dna minimal 5 karakter
    //         'level_id' => 'required|integer'         // level_id harus diisi dan berupa angka
    //     ]);
